...
class CodeRunner:
    ...
    def run_code_file(self, client_public_key: bytes, 
        ...
        run_code_file_process = subprocess.run(["./run_task_on_occlum.sh > output.txt"], cwd = CURRENT_PATH, capture_output = True, shell=True, text=True)
        # Give Permissions to output.txt
        give_permission_process = subprocess.run(["chmod", "777", "output.txt"], cwd = CURRENT_PATH) 
        if give_permission_process.returncode == 0:
            print("Give Code File Permission Success.")
        else:
            print("Give Code File Permission Fail.")
